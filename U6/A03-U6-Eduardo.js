        ...
        for (let i = 0; i < 2; i++) {
            ...
            for (let j = 0; j < 8; j++) {
                ...
                if(i == 1 && j == 5){
                    ...
                    td.setAttribute('id','motor');
                    td.setAttribute('class', this.model);
                }
                if(i == 1 && j == 6){
                    td.appendChild(document.createTextNode(this.velocidad+" Km/h"));
                };
...
let coche1 = new Vehiculo("Tesla", "X", "Rojo", "2022", 0),
    ...
    coche3 = new Vehiculo("Mclaren", "540c", "Rojo", "2021",100),
    coche4 = new Vehiculo("Tesla", "3", "Azul", "2020", 0),
    coche5 = new Vehiculo("BMW", "340", "Gris", "2005", 180);

let coches = [coche1 , coche2, coche3, coche4, coche5];
...
document.querySelectorAll('td[id="motor"]').forEach(motor => {
...
function arrancar(x) {
    coches.forEach(coche => {
        if(coche.model == x.className) {
            coche.arrancar();
            if(coche.arrancado() == "Arrancado") x.style.backgroundColor = "red";
